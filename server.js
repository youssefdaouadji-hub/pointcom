 ...
 * • Emails : notification commande (vous) + confirmation (client)
...
async function sendOrderEmail(session) {
  if (!transporter) {
    console.log('Email non configuré (ajoutez GMAIL_USER et GMAIL_APP_PASSWORD).');
    return;
  }
  ...
  try {
    ...
    console.log('Email de commande envoyé à', ORDER_EMAIL);
  } catch (e) {
...
// WEBHOOK STRIPE (avant express.json, corps brut)
app.post('/webhook', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  switch (event.type) {
    ...
    case 'checkout.session.async_payment_succeeded':
      ...
      sendOrderEmail(event.data.object);
      sendCustomerEmail(event.data.object);
      break;
    ...
    case 'invoice.payment_failed':
      console.log('Échec de prélèvement de la mensualité de maintenance.');
      break;
...
// RÉCAPITULATIF pour la page de confirmation
