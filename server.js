...
const upload = multer({
  ...
  limits: { fileSize: 8 * 1024 * 1024, files: 8 }, // 8 Mo max par image, 8 images
});

// Construit une session Stripe Checkout à partir des données de commande
async function createCheckoutSession({ formula, maintenance, name, email, project }) {
  const pack = PACKS[formula];
  if (!pack) throw new Error('Pack invalide.');

  const line_items = [{
    price_data: { currency: 'eur', product_data: { name: pack.label }, unit_amount: pack.amount },
    quantity: 1,
  }];
...
// Envoie le brief client + les images importées sur votre boîte
async function sendBriefEmail({ name, email, project, formula, maintenance, files }) {
  if (!transporter) {
    console.log('Brief reçu (email non configuré) —', files ? files.length : 0, 'image(s).');
    return;
  }
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;border:1px solid #eee;border-radius:12px;overflow:hidden">
      <div style="background:#070711;color:#fff;padding:20px 24px;font-size:18px;font-weight:bold">📥 Nouveau brief client — point com</div>
      <div style="padding:24px;font-size:15px;color:#222">
        <p><b>${name || '—'}</b> &lt;${email || '—'}&gt;</p>
        <p>Formule : <b>${formula || '—'}</b>${maintenance ? ' + maintenance' : ''}</p>
        <p style="white-space:pre-wrap">${(project || '(aucune description)').replace(/</g, '&lt;')}</p>
        <p style="color:#888">${attachments.length} image(s) jointe(s).</p>
      </div>
    </div>`;
  try {
    await transporter.sendMail({
      ...
      subject: `📥 Brief client — ${name || ''} (${formula || ''})`,
      html,
      attachments,
    });
    console.log('✉️  Brief + images envoyés à', ORDER_EMAIL, '(' + attachments.length + ' image(s))');
  } catch (e) {
    console.error('Erreur envoi brief :', e.message);
  }
}

// Commande avec images (multipart) : reçoit le brief + photos, puis crée le paiement
app.post('/order', upload.array('images', 8), async (req, res) => {
  try {
    const { formula, name, email, project } = req.body;
    const maintenance = req.body.maintenance === 'true' || req.body.maintenance === 'oui';
    if (!PACKS[formula]) return res.status(400).json({ error: 'Pack invalide.' });

    // 1) On vous transmet le brief + les images tout de suite
    await sendBriefEmail({ name, email, project, formula, maintenance, files: req.files });

    // 2) On crée la session de paiement
    const session = await createCheckoutSession({ formula, maintenance, name, email, project });
...
// Ancienne route JSON (sans images) — conservée par compatibilité
